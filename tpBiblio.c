...
printf("\n 12 - lister les emprunts en retard de ces morts"); //on suppose qu'un emprunt dure 7 jours.
...
switch(chx)
	...
	case 3 :	
				reponse = rechercherLivre(&B);
				if (!reponse)
					printf(" Ce livre n'est pas dans la bibliothèque !!");
				else{ 
					if (reponse == 1)
						printf(" Ce livre est présent dans la bibliothèque !!");
					else 
						printf(" Il y a %d livres portant ce nom dans la bibliothèque", reponse);
				}
	...
	case 4 :	reponse = rechercherAuteur(&B);
				if(!reponse)
					printf("Il n'y a pas de livre de cet auteur dans la bibliothèque !!");
	...
	case 5 :	reponse = supprimerLivre(&B);
				if (!reponse)
					printf("Impossible de supprimer ce livre!!");
				else 
	...
	case 7 :	reponse = rendreLivre(&B);
				if(!reponse)
					printf("Impossible de rendre ce livre");
				else
					printf("Livre rendu");
